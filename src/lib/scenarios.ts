...
import { Scenario } from './types'; // Ensure your types are imported correctly
...
export const scenarios: Scenario[] = [
  {
    ...
    steps: [
      {
        ...
        placeholder: 'e.g. LionKing',
        type: 'text',
        validate: (v) => v.trim().length < 2 ? 'Enter at least 2 characters' : null,
      },
      {
        id: 'twist',
        prompt: 'Name your favorite character from it.',
        hint: 'We combine words instead of misspelling! e.g. Simba',
      ...
      },
      {
        ...
        hint: 'Like your age when you first saw it, or a release year (e.g. 94).',
        placeholder: 'e.g. 94',
        type: 'number',
      ...
      {
        id: 'shield',
        prompt: 'Choose a special character to seal it.',
        hint: 'Your armor. Pick one: # @ ! $ & *',
        placeholder: 'e.g. !',
        type: 'special',
    ...
    buildStory: (a) => `You joined "${a[1]}" with the "${a[0]}", stamped it with ${a[2]}, and sealed the epic with "${a[3]}".`,
  },
  {
    id: 'hero-quest',
    ...
    steps: [
      ...
      {
        id: 'twist', 
        prompt: 'Pick an action verb — what do they do?',
        hint: 'e.g. Saves, Fights, Inspires.',
      ...
      {
        id: 'year',
        prompt: 'Pick a meaningful 4-digit year.',
        hint: 'A year related to them, or a year you\'ll never forget.',
    ...
    buildStory: (a) => `Your hero "${a[0]}" ${a[1].toLowerCase()} the day in ${a[2]}, marked by "${a[3]}".`,
  },
  {
    ...
    steps: [
      ...
      {
        id: 'twist',
        prompt: 'Name a favorite secret ingredient.',
        hint: 'Combine words for a stronger password! e.g. Garlic',
      ...
      {
        id: 'number',
        prompt: 'Pick a 2-digit cooking temperature or time.',
    ...
    buildPassword: (a) => `${a[1]}${a[0]}${a[2]}${a[3]}`,
    buildStory: (a) => `You mixed "${a[1]}" into your "${a[0]}", baked it for ${a[2]} minutes, and garnished it with "${a[3]}".`,
  },
  {
    id: 'travel-adventure',
    ...
    steps: [
      {
        ...
        hint: 'A city, country, or specific location.',
      ...
      {
        id: 'twist',
        prompt: 'Name a specific landmark, food, or activity from there.',
        hint: 'e.g. Sushi, Temple, Hike',
      ...
      {
        id: 'year',
        prompt: 'What 4-digit year did you go (or want to go)?',
        hint: 'e.g. 2019',
        placeholder: 'e.g. 2019',
        type: 'number',
        validate: (v) => /^\d{4}$/.test(v) ? null : 'Enter exactly 4 digits',
      },
      {
        id: 'shield',
    ...
    buildPassword: (a) => `${a[0]}${a[1]}${a[2]}${a[3]}`,
    buildStory: (a) => `You traveled to "${a[0]}", enjoyed the "${a[1]}", locked it in the year ${a[2]}, and anchored it with "${a[3]}".`,
  },
  {
    id: 'musical-journey',
    ...
    steps: [
      {
        id: 'band',
        prompt: 'Name an unforgettable band or artist.',
      ...
      {
        id: 'twist',
        prompt: 'What is your absolute favorite song by them?',
        hint: 'Combines nicely with the artist name. e.g. Bohemian',
      ...
      {
        id: 'number',
        prompt: 'Pick a 2-digit number.',
        hint: 'Like your age when you heard it, or a track number (e.g. 14).',
        placeholder: 'e.g. 14',
        type: 'number',
        validate: (v) => /^\d{2}$/.test(v) ? null : 'Enter exactly 2 digits',
      },
      {
        id: 'shield',
        prompt: 'Pick a symbol to represent the final beat.',
        hint: 'The final chord. Pick one: # @ ! $ & *',
        placeholder: 'e.g. &',
        type: 'special',
        validate: (v) => /^[!@#$%^&*()_+\-=\[\]{}|;:'",.<>?/\\`~]$/.test(v) ? null : 'Enter exactly 1 special character',
      },

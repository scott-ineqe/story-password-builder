...
export const scenarios: Scenario[] = [
    ...
    steps: [
      {
        ...
        hint: 'Think of something memorable.',
      ...
      },
      {
        ...
        hint: 'A number that means something to you.',
        placeholder: 'e.g. 88',
        type: 'number',
      ...
      {
        id: 'misspell',
        prompt: 'Now intentionally misspell the movie name.',
        hint: 'e.g. Lion King → Ly0nKing',
  ...
  },
  {
    ...
    steps: [
      {
        id: 'hero',
        prompt: 'Name a personal hero.',
        hint: 'Someone whose name sticks in your mind.',
      ...
      {
        id: 'action',
        prompt: 'Pick an action verb.',
        hint: 'What does your hero do? Fights, Saves...',
...
export function scorePassword(password: string): {
  ...
  let score = 0;
  const tips: string[] = [];
  if (password.length >= 8) score += 20; else tips.push('Make it at least 8 characters');
  if (password.length >= 12) score += 15;
  if (password.length >= 16) score += 10;
  if (/[a-z]/.test(password) && /[A-Z]/.test(password)) score += 15; else tips.push('Mix uppercase and lowercase');
  if (/\d/.test(password)) score += 15; else tips.push('Add some numbers');
  if (/[!@#$%^&*()_+\-=\[\]{}|;:'",.<>?/\\`~]/.test(password)) score += 15; else tips.push('Include a special character');
  if (!/(.)\1{2,}/.test(password)) score += 10; else tips.push('Avoid repeating characters');
...
export function simulateAttacks(password: string): AttackResult[] {
  ...
  const dictionaryAttack: AttackResult = {
    ...
    description: isCommon ? 'Found in common lists.' : 'Secure from common lists.',
  ...
  const bruteForceAttack: AttackResult = {
    ...
    description: secondsToCrack < 3600 ? 'Crackable quickly.' : 'Mathematically strong.',
